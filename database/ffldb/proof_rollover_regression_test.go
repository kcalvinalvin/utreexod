...
// TestProofStoreRejectsBackwardFileWrite ensures the proof write cursor never
// moves backward, since the single rollback cursor cannot safely describe an
// append to an older proof file.
func TestProofStoreRejectsBackwardFileWrite(t *testing.T) {
	...
	if err != nil {
		t.Fatalf("serialize first block: %v", err)
	}

	// Make the first block exactly fill block file 0, putting the second
	// block in file 1.
	pdb := idb.(*db)
	pdb.blkStore.maxBlockFileSize = uint32(len(firstBlockBytes) + 12)
	err = idb.Update(func(tx database.Tx) error {
		if err := tx.StoreBlock(blocks[1]); err != nil {
			return err
		}
		return tx.StoreBlock(blocks[2])
	})
	if err != nil {
	...
	want := []byte("proof in block file one")
	err = idb.Update(func(tx database.Tx) error {
		return tx.StoreUtreexoProof(blocks[2].Hash(), want)
	})
	if err != nil {
		t.Fatalf("store forward proof: %v", err)
	}

	// A later proof for block file 0 must fail instead of moving the single
	// proof cursor backward and making rollback/reconciliation ambiguous.
	err = idb.Update(func(tx database.Tx) error {
		return tx.StoreUtreexoProof(blocks[1].Hash(), []byte("late proof"))
	})
	if err == nil {
		t.Fatal("backward proof write unexpectedly succeeded")
	}
	dbErr, ok := err.(database.Error)
	if !ok || dbErr.ErrorCode != database.ErrDriverSpecific {
		t.Fatalf("backward proof write error: got %T %v, want ErrDriverSpecific",
			err, err)
	}

	// The rejected write must not disturb the proof already committed in
	// the current file.
	err = idb.View(func(tx database.Tx) error {
		got, err := tx.FetchUtreexoProof(blocks[2].Hash())
		if err != nil {
			return err
		}
		if !bytes.Equal(got, want) {
			return fmt.Errorf("forward proof changed: got %x, want %x", got, want)
		}
		return nil
	})
	if err != nil {
		t.Fatalf("verify forward proof: %v", err)
	}
}
