...
func pruneChecks(db database.DB) error {
	...
	if beenPruned && cfg.UtreexoProofIndex {
		return fmt.Errorf("--utreexoproofindex cannot be enabled as the node has been "+
			"previously pruned. You must delete the files in the datadir: \"%s\" "+
			"and sync from the beginning to enable the desired index", cfg.DataDir)
	}
	if beenPruned && cfg.FlatUtreexoProofIndex {
	...
	// If we've previously been pruned and the utreexo proof index isn't present, it means that the
	// user wants to enable the utreexo proof index after the node has already synced up and been
	// pruned.
	if beenPruned && !indexers.UtreexoProofIndexInitialized(db) && cfg.UtreexoProofIndex {
		return fmt.Errorf("utreexo proof index cannot be enabled as the node has been "+
			"previously pruned. You must delete the files in the datadir: \"%s\" "+
			"and sync from the beginning to enable the desired index. You may "+
			"start the node up without the --utreexoproofindex flag", cfg.DataDir)
	}
	...
	if beenPruned && !indexers.FlatUtreexoProofIndexInitialized(db) && cfg.FlatUtreexoProofIndex {
		return fmt.Errorf("flat utreexo proof index cannot be enabled as the node has been "+
			"previously pruned. You must delete the files in the datadir: \"%s\" "+
			"and sync from the beginning to enable the desired index. You may "+
			"start the node up without the --flatutreexoproofindex flag", cfg.DataDir)
	}
